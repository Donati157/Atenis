-- HOTFIX v2 — corrige `column reference "id" is ambiguous`.
-- Idempotente. Cola TUDO e roda.

drop function if exists public.apply_signup_intent(text,text,text,text[],text[]) cascade;
drop function if exists public.apply_signup_intent(text,text,text,text[],text[],text) cascade;
