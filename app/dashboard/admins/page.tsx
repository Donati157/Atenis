...
export default async function AdminsPage() {
  ...
  const { data: admins, error } = await supabase.rpc("list_admins_for_admin")

  const list: Admin[] = (admins as Admin[] | null) ?? []
    ...
    <div className="min-h-screen bg-background">
      ...
      <div className="container mx-auto px-4 py-6 sm:py-8">
        <div className="max-w-5xl mx-auto space-y-5 sm:space-y-6">
          <div>
            <h1 className="text-2xl sm:text-3xl font-bold font-display">Admins</h1>
            <p className="text-sm sm:text-base text-muted-foreground mt-1">
              Gestão de administradores. Apenas o admin principal pode promover ou rebaixar admins.
            </p>
          </div>

          {error ? (
            <div className="bg-destructive/10 border border-destructive/30 rounded-lg p-4 text-sm text-destructive">
              Erro ao carregar admins: {error.message}. Verifique se a migration{" "}
              <code className="font-mono">019_super_admin_role_change.sql</code> foi executada no Supabase.
            </div>
          ) : (
